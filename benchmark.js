...
function measure(block) {
  ...
  if (has_gc_access) {
    ...
    if (typeof sm.vsize === "number") {
      ...
      msg +=
        "   vsize:      " +
        mvsized.toSignedHuman(1024) +
        " kB -- (" +
        sm.vsize.toHuman(1024) +
        " kB -> " +
        em.vsize.toHuman(1024) +
        " kB)\n";
    }
    msg +=
      "   heap total: " +
      mhtotd.toSignedHuman(1024) +
      " kB -- (" +
      sm.heapTotal.toHuman(1024) +
      " kB -> " +
      em.heapTotal.toHuman(1024) +
      " kB)\n" +
      "   heap used:  " +
      mhusedd.toSignedHuman(1024) +
      " kB -- (" +
      sm.heapUsed.toHuman(1024) +
      " kB -> " +
      em.heapUsed.toHuman(1024) +
      " kB)\n\n";
  }
...
measure(function() {
  ...
  for (var i = 1, L = N + 1; i < L; ++i) c.get("key" + i, i);
});
...
measure(function() {
  ...
  for (var i = 1, L = N + 1; i < L; ++i) c.get("key" + i);
});
...
measure(function() {
  ...
  for (var i = 1, L = N + 1; i < L; ++i) c.get("xkey" + i);
});
...
measure(function() {
  ...
  for (var i = N; --i; ) c.set("key2_" + i, i);
});

measure(function() {
  // 6. shift head -> tail
  //    Remove all entries going from head to tail
  for (var i = 1, L = N + 1; i < L; ++i) c.shift();
});
